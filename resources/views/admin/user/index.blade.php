            ...
            <tbody>
                @foreach ($user as $role => $data)
                  <tr>
                    ...
                    <td>{{ $data->count() }}</td>
                    <td>
                      <a href="{{ route('action-view', Crypt::encrypt($role)) }}" class="btn btn-info btn-sm"><i class="nav-icon fas fa-search-plus"></i> &nbsp; Ditails</a>
                    </td>
                  </tr>
                @endforeach
            </tbody>
  ...
  <script>
    $(document).ready(function(){
        $('#role').change(function(){
            ...
            if (kel == "Karyawan") {
              $("#noId").html('<label for="nomor">Nomor Id Card</label><input id="nomor" type="text" maxlength="5" onkeypress="return inputAngka(event)" placeholder="No Id Card" class="form-control" name="nomor" autocomplete="off">');
